/* ============================================================================
   ...
   CARRIED FROM THE MOCKUP SET UNCHANGED
   -------------------------------------

   This file arrived here from `mockups/moments.js` with its arithmetic, its
   ordering and its token reads intact. What changed is the wrapper: the
   mockups' immediately-invoked function that scanned the document on load is
   now an ES module the application calls, because the application supplies
   real data rather than a static page supplying sample attributes.

   Nothing between those two braces was retuned. The timings still come from
   `tokens.css`, which still cites the V1 file and line each group came from,
   so retuning a duration is still editing one number in that file.
   ========================================================================= */
...
export { play as playMoments, readingGeometry };
