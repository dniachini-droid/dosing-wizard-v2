...
export const MUTATIONS = [
  ...
  */
  ...
  /* ---- ROUND FIVE: Setup's controls (findings 16, 17, 18, 24) -------- */
  {
    id: "AM-R38",
    why: "a saved value goes back to looking like an empty-headed input, so nothing on the screen says the save took (finding 16)",
    file: "app/src/components/Setup.jsx",
    find: "  const tankLocked = tankSaved && !tankEditing;",
    replace: "  const tankLocked = false;",
    breaks: ["SC-01"],
  },
  {
    id: "AM-R39",
    why: "pressing Edit writes to the record, so unlocking a field to look at it changes what the app is using",
    file: "app/src/components/Setup.jsx",
    find: "          onEdit={() => setTankEditing(true)}",
    replace: "          onEdit={() => { saveFacts([\"netVolumeL\"]); setTankEditing(true); }}",
    breaks: ["SC-02"],
  },
  {
    id: "AM-R40",
    why: "the delivered dose is locked too, putting a tap in front of the app's most repeated action to solve a problem it does not have",
    file: "app/src/components/Setup.jsx",
    find: "              <DeliveredDoseField standing={standingDose} onSave={onSetDeliveredDose} />",
    replace: "              <DeliveredDoseField locked={tankLocked} standing={standingDose} onSave={onSetDeliveredDose} />",
    breaks: ["SC-03"],
  },
  {
    id: "AM-R41",
    why: "the short box is spelled as two widths on one element, which Tailwind resolves by stylesheet order rather than attribute order — so it is a coin toss whether a three-digit volume gets a phone-wide box again (finding 17)",
    file: "app/src/components/DoseExpectation.jsx",
    find: 'export const shortInputCls = inputCls.replace("w-full", "w-24") + " text-right";',
    replace: 'export const shortInputCls = inputCls + " w-24 text-right";',
    breaks: ["SC-04"],
  },
  {
    id: "AM-R42",
    why: "the range widths are graded at the wrong boundaries, so a range the owner called acceptable is reported as tight",
    file: "app/src/present/range-grade.js",
    find: '  if (width < 0.5) return { key: "tight", tone: "#0B7C86" };',
    replace: '  if (width <= 0.5) return { key: "tight", tone: "#0B7C86" };',
    breaks: ["SC-05"],
  },
  {
    id: "AM-R43",
    why: "the bar grades every parameter by default, so alkalinity's thresholds are applied to calcium and magnesium — which the owner explicitly said they do not transfer to",
    file: "app/src/components/RangeSlider.jsx",
    find: '  unit = "dKH", graded = false, disabled = false,',
    replace: '  unit = "dKH", graded = true, disabled = false,',
    breaks: ["SC-06"],
  },
  {
    id: "AM-R44",
    why: "the range handles can cross, so the bottom of the range ends up above the top of it",
    file: "app/src/components/RangeSlider.jsx",
    find: "  const setLo = (v) => onChange(Math.min(v, hi - step), hi);",
    replace: "  const setLo = (v) => onChange(v, hi);",
    breaks: ["SC-07"],
  },
  {
    id: "AM-R45",
    why: "calcium goes back to mg/L, which is not the word on any test kit, any bottle, or in any conversation between reefkeepers (finding 24)",
    file: "app/src/store/ledger.js",
    find: '{ key: "CA", unit: "ppm", decimals: 0, tone: "ca", assessed: false },',
    replace: '{ key: "CA", unit: "mg/L", decimals: 0, tone: "ca", assessed: false },',
    breaks: ["SC-08", "SC-09"],
  },
  {
    id: "AM-R46",
    why: "the unit change becomes a CONVERSION, so the keeper's whole imported history silently becomes a thousand times wrong",
    file: "app/src/store/ledger.js",
    find: '  { key: "PO4", unit: "ppm", decimals: 3, tone: "po4", assessed: false },',
    replace: '  { key: "PO4", unit: "ppm", decimals: 3, tone: "po4", assessed: false, scale: 1000 },',
    breaks: ["SC-10"],
  },
