...
export const MUTATIONS = [
  ...
  {
    id: "AM-88",
    why: "a reading with a local clock reading is promoted to an absolute instant using the device's timezone today, which is the second fabrication the contract forbids absolutely",
    file: "app/src/store/import-v1.js",
    find: 'import { dateOnly, localTimeZoneUnknown, reconstructedInstant, resolveLocalInZone } from "./time.js";',
    replace: 'import { dateOnly, exactInstant as localTimeZoneUnknown, reconstructedInstant, resolveLocalInZone } from "./time.js";',
    breaks: ["IMP-02", "IMP-03", "IMP-04", "IMP-20"],
  },
  {
    id: "AM-89",
    why: "the local-time constructor quietly computes an instant, so every imported time acquires an offset nobody wrote down",
    file: "app/src/store/time.js",
    find: "    localTime: String(localTime).slice(0, 5),\n    /* No absoluteInstant, for the same reason `dateOnly` has none. */",
    replace:
      "    localTime: String(localTime).slice(0, 5),\n" +
      '    absoluteInstant: String(localDate).slice(0, 10) + "T" + String(localTime).slice(0, 5) + ":00Z",',
    breaks: ["IMP-02", "IMP-03"],
  },
  {
    id: "AM-90",
    why: "an imported time is left unfrozen, so a later line can add the instant the constructor refused to invent",
    file: "app/src/store/time.js",
    find: '  if (!localDate || !localTime) throw new Error(t("err.exactInstantNeedsBoth"));\n  return Object.freeze({',
    replace: '  if (!localDate || !localTime) throw new Error(t("err.exactInstantNeedsBoth"));\n  return ({',
    breaks: ["IMP-04"],
  },
  {
    id: "AM-91",
  ...
  {
    id: "AM-122",
    why: "a date-only reading is reconstructed too, so 325 readings that never had a time acquire midnight in the keeper's stated zone",
    file: "app/src/store/import-v1.js",
    find: "  if (!row.time) return dateOnly(row.date);\n  if (reconstruction && reconstruction.zoneId) {",
    replace: "  if (!row.time && !reconstruction) return dateOnly(row.date);\n  if (!row.time) row = { ...row, time: \"00:00\" };\n  if (reconstruction && reconstruction.zoneId) {",
    breaks: ["IMP-23"],
  },
  {
    id: "AM-123",
    why: "one offset is applied to the whole history, so every reading before April carries August's offset and is an hour out",
    file: "app/src/store/time.js",
    find: "  const resolved = resolveLocalInZone(localDate, localTime, zoneId);\n  if (!resolved) return null;",
    replace: "  const resolved = { utcMs: Date.parse(`${localDate}T${localTime}:00Z`) - 600 * 60000, offsetMinutes: 600 };",
    breaks: ["IMP-24", "IMP-25"],
  },
  {
    id: "AM-124",
    why: "an hour daylight saving skipped or repeated is reconstructed anyway, picking one of two answers or inventing one that never happened",
    file: "app/src/store/time.js",
    find: "  if (distinct.length !== 1) return null;\n  return { utcMs: distinct[0], offsetMinutes: (naive - distinct[0]) / 60000 };",
    replace: "  const pick = distinct.length ? distinct[0] : naive - first * 60000;\n  return { utcMs: pick, offsetMinutes: (naive - pick) / 60000 };",
    breaks: ["IMP-25"],
  },
  {
    id: "AM-125",
    why: "the stronger provenance is written without the reconstruction that justifies it, which is half of what the canon requires",
    file: "app/src/store/time.js",
    find: "  if (!reconstruction || !reconstruction.basis || !reconstruction.statedAt) {",
    replace: "  if (false) {",
    breaks: ["IMP-26"],
  },
  {
    id: "AM-126",
    why: "the reconstruction is applied but not recorded on the record, so nothing downstream can see the time was worked out rather than measured",
    file: "app/src/store/time.js",
    find: "    reconstruction: Object.freeze({ ...reconstruction, zoneId, offsetMinutes: off }),",
    replace: "",
    breaks: ["IMP-22"],
  },
  {
    id: "AM-127",
    why: "the timed readings are reconstructed even when the keeper stated no zone, using whatever the device's offset happens to be — the fabrication the canon forbids by name",
    file: "app/src/store/import-v1.js",
    find: "  if (reconstruction && reconstruction.zoneId) {",
    replace: '  if (true) {\n    reconstruction = reconstruction || { zoneId: "UTC", basis: "X", statedAt: "2026-01-01T00:00:00Z" };',
    breaks: ["IMP-27"],
  },
  {
    id: "AM-128",
  ...
  },
  {
    ...
    file: "app/src/store/import-v1.js",
    find: "      ...(volumeL > 0 ? { changedFraction: r.litres / volumeL } : {}),",
    replace: "",
    breaks: ["IMP-36"],
  },
  {
