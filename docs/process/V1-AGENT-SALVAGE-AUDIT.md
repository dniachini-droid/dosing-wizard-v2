...
| 10 | `domain-verifier` | Chemistry questions; sourced vs design distinction | *none declared* (inherits all, while its description claims "Reports only") | Two-layer report | Heavy: `legacy/` simulator as oracle, named V1 functions, specific corrected-error figures, owner's private data path | — | `REBUILD_THE_IDEA` |
