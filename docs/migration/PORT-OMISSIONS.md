...
| **Correcting or deleting a reading** | `RESTORED (round three, stage 3)` | V1's `WaterLog` carried the edit and delete affordances and did not cross; V2's `screens/entrydetail.js` went with the V2 interface. **There is no way to fix a mistyped reading in this build.** | A correction sheet that calls `store.ledger.append` with `supersedes`. The store's rule — a correction may not improve a record's time provenance — already holds and is tested (`PORT-12`, `IMP-35`); only the surface is missing. **This is the most useful thing on this list.** **Built in round three:** `correctReading` in `app/src/lib/record.js`, the sheet in `app/src/components/CorrectReadingSheet.jsx`, reached from the reading list beneath the chart in the parameter view. The original is superseded, never overwritten and never deleted; taking a reading out annotates it invalid and keeps it. Both consequences are stated on the sheet before either runs. `COR-01`…`COR-03`, with mutations `AM-C1`…`AM-C3`. |
...
|---|---|---|---|
| **Test mode** — setting the assessment instant by hand, stepping it, and the marker pinned above every screen | `RESTORED (round three, item 9)` | The mechanism is intact and untouched: `app/src/store/mode.js`, the clock in `app/src/store/time.js`, and `TM-01`…`TM-25` all still run. **There is no way into it.** `TM-24` asserts exactly this rather than passing quietly | A section in Setup, and the marker in the shell. The brief puts test mode behind Settings rather than in the navigation, which is where it was. **Built in round three:** `app/src/components/TestMode.jsx`, in Setup, with the marker in the shell. The wiring mattered more than the screen — `App.jsx` called `createStore()` unconditionally, so even a wired screen would have read the REAL tank in test mode; the shell now takes its store from `storeForMode` and its clock from `applyClock`. `TM-24` is inverted to pin both. |
| **Bulk entry from a pasted series** (`store/seed.js`) | `RESTORED (round three, item 9)` | Same: the module survives, the surface does not. Its "refused outside test mode" guard has nothing left to guard, which `TM-12` now records | Reachable from the test-mode section above. **Built in round three**, inside that section, with the grammar behind a disclosure and a preview before anything is written. |
...
| **The offline shell** — the service worker | `RESTORED (round three, item 10)` | `app/sw.js` precached a file list that no longer exists, and was deleted rather than left registering a stale cache. **The app is no longer installable or offline-capable.** | The build now produces a hashed bundle, so the precache list has to be generated from it rather than written by hand. `vite-plugin-pwa`, which V1 used, is the obvious route. **Built in round three:** `app/sw.js` with the precache list generated at build time by a plugin in `vite.config.js`, which also reads the engine's own module list out of `worker.js` rather than restating it. It precaches the shell AND the fourteen engine Python files AND the frozen catalogue — an app that opens offline and then cannot answer is a worse failure than one that does not open. **Verified by running it with the network off**, as the round-three brief required: `tools/app/check-offline.mjs` loads the built app in a real browser, then takes the origin away — offline context AND the server shut down — and asserts the app renders, zero requests reach a server, and `engine.py` and the catalogue read back from cache. |
